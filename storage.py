...
class RolloutStorage(object):
    ...
    def _discount_rewards(self, final_value, discount=0.99):
        ...
        if self.is_cuda:
            returns = torch.zeros(self.rollout_size + 1, self.num_envs).cuda()
        else:
            returns = torch.zeros(self.rollout_size + 1, self.num_envs)

        mask = (1 - self.dones[-1]).bool()
        returns[-1] = self._generate_buffer(self.num_envs).masked_scatter(mask, final_value)
        ...
        for i in reversed(range(self.rollout_size)):
            ...
            # todo: add GAE
            R = self._generate_buffer(self.num_envs).masked_scatter(mask,
                                                                    self.rewards[i] + discount * returns[i+1])
    ...
    def a2c_loss(self, final_value, entropy):
        ...
        returns = self._discount_rewards(final_value)
        advantage = returns[:-1] - self.values
        ...
        return loss, value_loss, policy_loss, -self.entropy_coeff * entropy
